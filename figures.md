...
**Figure S3.** Heatmaps comparing SlaClip with its closest adaptive clipping baseline, Adap-Clip, on CIFAR-10. Results are shown for three initial clipping thresholds, $C_0 \in \{0.1, 1, 10\}$, under both constant and cosine learning-rate schedules. Each cell reports the final test accuracy for one $(\text{lr}, C_0)$ configuration.
